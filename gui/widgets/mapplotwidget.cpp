...
void MapPlotWidget::plotRaw(Mpa2dHist *hist){
    ...
    for (int xIndex=0; xIndex<hist->mXSize; ++xIndex)
    {
      for (int yIndex=0; yIndex<hist->mYSize; ++yIndex){
          int x,y;
          colorMap->data()->coordToCell((double)xIndex,(double)yIndex,&x,&y);
          colorMap->data()->setCell(x,y,hist->mRawHist(yIndex,xIndex));
      }
    }
    // add a color scale:
...
//    }
    ...
    crossX->start->setCoords(hist->mCenter(0),hist->mCenter(1)-1);
    crossX->end->setCoords(hist->mCenter(0),hist->mCenter(1)+1);
    QCPItemLine *crossy = new QCPItemLine(ui->customPlot);
    crossy->start->setCoords(hist->mCenter(0)-1,hist->mCenter(1));
    crossy->end->setCoords(hist->mCenter(0)+1,hist->mCenter(1));
...
void MapPlotWidget::plotCentered(Mpa2dHist *hist){
    ...
    QCPColorMap *colorMap = new QCPColorMap(ui->customPlot->xAxis, ui->customPlot->yAxis);
    colorMap->data()->setSize(hist->mXSize,hist->mYSize);
    colorMap->data()->setRange(QCPRange(0,799),QCPRange(0,799));
    //colorMap->data()->setRange(QCPRange(0,1024),QCPRange(0,1024));
    for (int xIndex=0; xIndex<800; ++xIndex)
    {
      for (int yIndex=0; yIndex<800; ++yIndex){
          int x,y;
          colorMap->data()->coordToCell((double)xIndex,(double)yIndex,&x,&y);
          colorMap->data()->setCell(x,y,hist->mCenteredHist(yIndex,xIndex));
      }
    }
    // add a color scale:
    QCPColorScale *colorScale = new QCPColorScale(ui->customPlot);
    ...
    //mark center
    QCPItemLine *crossX = new QCPItemLine(ui->customPlot);
    crossX->start->setCoords(400,400-1);
    crossX->end->setCoords(400,400+1);
    QCPItemLine *crossy = new QCPItemLine(ui->customPlot);
    crossy->start->setCoords(400-1,400);
    crossy->end->setCoords(400+1,400);
